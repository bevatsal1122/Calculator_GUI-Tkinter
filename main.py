...
class Calculator:
    ...
    def create_display_frame(self):
        frame = Frame(self.window, height=260)
    ...
    def add_to_current_label(self, value):
        if self.c_expression.find("Error") == -1 and self.t_expression.find("Error") == -1:
            if (len(self.c_expression) <= 12) and (len(self.c_expression) <= 10 or self.c_expression[-1] == '+' or
                                                   self.c_expression[-1] == '-' or self.c_expression[-1] == '*' or
                                                   self.c_expression[-1] == '/'):
    ...
    def get_square_root(self):
        if self.c_expression.find("Error") == -1 and self.t_expression.find("Error") == -1:
            ...
            if check < 0:
                ...
                self.c_expression = self.c_expression[:10]
        self.update_current_label()
